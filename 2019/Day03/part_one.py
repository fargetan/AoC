...
# take list of strings and split into little lists
# 

_Map1_ = {}
_Map2_ = {}

#instructions1 = [ ['R',75], ['D',30], ['R',83], ['U',83], ['L',12], ['D',49], ['R',71],['U',7], ['L',72] ]
#instructions2 = [ ['U',62], ['R',66], ['U',55], ['R',34], ['D',71], ['R',55], ['D',58],['R',83] ]

#instructions1 = [ ['R',8], ['U',5], ['L',5], ['D',3]]
#instructions2 = [ ['U',7], ['R',6], ['D',4], ['L',4]]

#instructions1 = 'R8,U5,L5,D3'.split(',')
#instructions2 = 'U7,R6,D4,L4'.split(',')


"""
# lay first wire
def layWire(instructions, wireMap):
  position = [0, 0]
  for move in instructions:
    if move[0] == 'R':
      for i in range(move[1]):
        position[0] += 1
        wireMap[str(position)] = [1, abs(position[0]) + abs(position[1])]
    if move[0] == 'D':
      for i in range(move[1]):
        position[1] -= 1
        wireMap[str(position)] = [1, abs(position[0]) + abs(position[1])]
...
# wire1
#layWire(instructions1, _Map1_)
# wire2
#layWire(instructions2, _Map2_)
#print(_Map1_)
#print(_Map2_)
#find crosses

layWire(wire1, _Map1_)
layWire(wire2, _Map2_)

crosses = {}
for item in _Map1_:
  try:
    if _Map2_[str(item)][0] == 1:
      #print('cross')
      #print(item)
      crosses[item] = _Map2_[item][1]
  except KeyError:
    continue
...
shortest = 1000
for item in crosses:
  if crosses[item] < shortest:
    shortest = crosses[item]
